...
class stack_buf {
 public:
  ...
  stack_buf& operator=(T1&& other) {
    ...
    if (other.vector_used())
      ...
      std::copy_n(other._stack_array.begin(), other._stack_size,
                  _stack_array.begin());
    return *this;
  }
 ...
 private:
  ...
  inline bool vector_used() const { return !(_v.empty()); }
...
class stack_devicebuf : public std::streambuf {
 public:
  ...
  stack_devicebuf(const stack_devicebuf& other)
      : std::basic_streambuf<char>(), _stackbuf(other._stackbuf) {}

  stack_devicebuf(stack_devicebuf&& other)
      : std::basic_streambuf<char>(), _stackbuf(std::move(other._stackbuf)) {
    other.clear();
  }
  ...
  const stackbuf_t& buf() const { return _stackbuf; }
  std::size_t size() const { return _stackbuf.size(); }

  void clear() { _stackbuf.clear(); }
...
class rpj_oss : public std::ostream {
 public:
  ...
  const stack_devicebuf::stackbuf_t& buf() const { return _dev.buf(); }

  std::size_t size() const { return _dev.size(); }
  void clear() { _dev.clear(); }
